...
$(document).ready(function () {
	...
	CMS.Modal = new CMS.Class({
		...
		_setButtons: function (iframe) {
			...
			var buttons = row.find('input[type="submit"], a, button');
			var comment = row.find('#revision_comment');
			...
			comment.each(function (index, item) {
				item = $(item);
				var el = $('<input type="text" placeholder="'+item.attr('placeholder')+'" style="border: 1px solid #CCC; margin: 3px 6px; padding: 2px 6px; width: 400px;"/>');
				render.append(el);
			});
